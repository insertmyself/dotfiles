...
wallpaper_random() {
    ...
    awww img "${WALLPAPER_RANDOM}" --transition-fps 60 --transition-type outer --transition-pos center --transition-duration 1
    notify-send "Wallpaper Randomizer" "${WALLPAPER_FILE} wallpaper applied"
}

load_wallpaper() {
    if [[ -f ${WALLPAPER_TEMPF} ]]; then
        awww img ${WALLPAPER_TEMPF} --transition-fps 60 --transition-type grow --transition-pos center --transition-duration 1
    else
        awww img $(awk -F'=' "/^\\[Wallpaper\\]/{flag=1;next}/^\\[/{flag=0}flag && \$1~/LastWallpaper/{print \$2}" "$HOME/.config/switar/config.ini" | tr -d '[:space:]') --transition-fps 60 --transition-type grow --transition-pos center --transition-duration 1
    fi
}
